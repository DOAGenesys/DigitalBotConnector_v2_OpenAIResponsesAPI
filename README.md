...
1. Set environment variables in Vercel dashboard, including GENESYS_CONNECTION_SECRET, OPENAI_API_KEY (though passed via headers, but for testing), REDIS_URL if using Redis, etc.
2. For Redis, use Upstash or similar provider compatible with Vercel, set REDIS_URL and SESSION_STORE_TYPE=redis.
3. Deploy via git push to Vercel-linked repository.
4. For local development, use `npm run dev`. Ensure .env.local has necessary vars like GENESYS_CONNECTION_SECRET.

## 10. Genesys Cloud Setup
...
### Bot Connector URI: 
Your Vercel deployment URL, e.g. `https://your-app.vercel.app/botconnector`
